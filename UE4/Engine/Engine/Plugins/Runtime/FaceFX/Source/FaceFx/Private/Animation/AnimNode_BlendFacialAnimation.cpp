...
DECLARE_CYCLE_STAT(TEXT("Blend Facial Animation"), STAT_FaceFXBlend, STATGROUP_FACEFX);
DECLARE_CYCLE_STAT(TEXT("Blend Facial Animation - Load"), STAT_FaceFXBlendLoad, STATGROUP_FACEFX);
...
void FAnimNode_BlendFacialAnimation::LoadFaceFx(UAnimInstance* animInstance)
{
	...
	if(USkeletalMeshComponent* component = animInstance ? animInstance->GetSkelMeshComponent() : nullptr)
	{
		...
		{
			if(UFaceFxCharacter* fxChar = faceFxComp->GetCharacter(component))
			{
			...
			else
			{
				...
				m_faceFxCharacterLoadingCompleted = !faceFxComp->IsLoadingFaceFxCharacterAsync();
			}
		}
	}
}
