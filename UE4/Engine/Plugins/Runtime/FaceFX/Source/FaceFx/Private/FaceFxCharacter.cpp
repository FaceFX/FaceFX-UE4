...
void UFaceFxCharacter::Tick(float deltaTime)
{
	...
	checkf(m_canPlay, TEXT("Internal Error: FaceFX character is not allowed to tick."));
...
bool UFaceFxCharacter::Restart()
{
	...
	if(!IsLoaded())
	{
		UE_LOG(LogFaceFx, Warning, TEXT("UFaceFxCharacter::Restart. FaceFX character not loaded. Asset: %s"), *GetNameSafe(m_faceFxSet));
		return false;
	}
...
#endif //Y_FACEFX_WITHBONEFILTER
{
	...
	ffx_context_t context = FFaceFxContext::CreateFaceFxContext();
...
ffx_anim_handle_t* UFaceFxCharacter::LoadAnimation(const FFaceFxAnimData& animData)
{
	...
	ffx_context_t ctx = FFaceFxContext::CreateFaceFxContext();
